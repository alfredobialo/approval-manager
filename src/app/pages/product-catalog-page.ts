...
@Component({
  ...
  template: `
    <div class="h-full">
      ...
      <div class="xl:grid xl:grid-cols-3 xl:grid-flow-row xl:gap-6 ">
        @for(p of products() ;track p.id){
          <div class="mb-[100px] rounded-2xl border-2 border-2 border-gray-300 dark:border-surface-700 px-6 hover:shadow-xl hover:dark:shadow-gray-600 duration-300 ">
            <div class="h-[200px] w-[150px] xl:h-[360px] xl:w-[310px]  ">
              <img [ngSrc]="p.imageUrl" width="310" height="360" alt=""priority>
            </div>
            <a  [routerLink]="['product', p.id]"  class="text-2xl dark:text-blue-400 text-blue-600 underline">{{ p.name }}</a>
